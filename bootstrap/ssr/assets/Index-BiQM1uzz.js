...
import DataTable from "./DataTable-DHQZECI5.js";
...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    watch(
      ...
      debounce((value) => {
        router.get(
          route("admin.settings.versions.index"),
          { search: value },
    ...
    const deletePaymentMethod = (item) => {
      router.delete(route("admin.settings.versions.destroy", item.id), {
        onBefore: () => confirm($t("Are you sure you want to delete this payment method?"))
      });
    };
    const handleCreate = () => {
      router.visit(route("admin.settings.versions.create"));
    };
    const actionButtons = [
      {
        label: "create",
