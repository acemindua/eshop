        ...
        sidenav: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (auth_user.value) {
              ...
              _push2(`</div></div><p class="font-semibold"${_scopeId}>${ssrInterpolate(auth_user.value.full_name)}</p><p class="flex items-center justify-center bg-gray-600 rounded-full h-5 px-2 relative"${_scopeId}><span class="text-xs text-gray-50 -top-[2px] relative"${_scopeId}>${ssrInterpolate(auth_user.value.role)}</span></p><p class="text-gray-400 truncate w-full text-center"${_scopeId}>${ssrInterpolate(auth_user.value.email)}</p><div class="border-t border-dashed border-brand mt-2 w-[50%]"${_scopeId}></div></div></li>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<li${_scopeId}>`);
