import { ref, onMounted, onUnmounted, mergeProps, useSSRContext, computed, unref } from "vue";
...
const _sfc_main = {
  ...
  __ssrInlineRender: true,
  props: {
    // Очікуємо загальний об'єкт даних від Edit.vue
    data: { type: Object, default: () => null },
    // Прапорець режиму редагування сторінки
    isEditing: { type: Boolean, default: false },
    // Системні дефолтні значення для поліморфного зв'язку
  ...
  setup(__props) {
    const props = __props;
    const images = ref([]);
    const modelId = computed(() => props.data?.item?.id);
    const meta = computed(() => ({
    ...
    onMounted(async () => {
      if (!props.isEditing || !modelId.value) {
        if (props.data?.item?.sorted_images) {
          images.value = [...props.data.item.sorted_images];
        }
        return;
      }
      try {
        const params = new URLSearchParams();
        params.append("model_type", props.modelType);
        params.append("model_id", String(modelId.value));
        params.append("collection", props.collection);
        const response = await getMedia("/api/media", { params });
        images.value = response?.media || props.data?.item?.sorted_images || [];
      } catch (e) {
        console.error("API fetch failed, falling back to inert data", e);
        if (props.data?.item?.sorted_images) {
          images.value = [...props.data.item.sorted_images];
        }
      }
    ...
    const handleReorder = async (newImages) => {
      ...
      if (Object.keys(order).length > 0) {
        ...
        await sortMedia("/api/media/sort", sortPayload);
      }
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(uploadErrors) && Object.keys(unref(uploadErrors)).length > 0) {
        _push(`<div class="p-3 bg-red-50 border border-red-200 rounded-none"><!--[-->`);
        ssrRenderList(unref(uploadErrors), (errors, field) => {
          _push(`<p class="text-red-600 text-xs font-mono">${ssrInterpolate(errors[0])}</p>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(` ${ssrInterpolate(props.data?.item?.sorted_images)} `);
