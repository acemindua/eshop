...
import { _ as _sfc_main$6, a as _sfc_main$7, b as _sfc_main$8 } from "./DefaultLayout-BA31SkrW.js";
...
import "./UserLoginRegisterForm-BxqOXTE8.js";
import "./AuthModal-CPhI-wdS.js";
...
const MediaSlider = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-3b9a1d4f"]]);
const _sfc_main$4 = {
  __name: "ReviewForm",
  __ssrInlineRender: true,
  props: {
...
const _sfc_main$3 = {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "review-list" }, _attrs))} data-v-a4346698><h2 class="text-2xl font-semibold mb-4 text-gray-800" data-v-a4346698> Відгуки користувачів </h2>`);
      if (unref(api).loading.value) {
        _push(`<div class="text-center text-gray-600 py-4" data-v-a4346698> Завантаження відгуків... </div>`);
      } else if (reviews.value.length === 0) {
        _push(`<div class="text-center text-gray-600 py-4" data-v-a4346698> Ще немає відгуків. Будьте першим! </div>`);
      } else {
        _push(`<div data-v-a4346698><!--[-->`);
        ssrRenderList(reviews.value, (review) => {
          _push(`<div class="bg-white p-4 rounded-lg shadow-sm mb-4 border border-gray-200" data-v-a4346698><div class="flex items-center justify-between mb-2" data-v-a4346698><div class="flex items-center" data-v-a4346698>`);
          _push(ssrRenderComponent(_sfc_main$6, {
            "initial-rating": review.overall_rating,
            "read-only": true,
            class: "text-lg"
          }, null, _parent));
          _push(`<span class="ml-3 text-sm text-gray-600" data-v-a4346698> від ${ssrInterpolate(review.user ? review.user.name : "Анонім")} на ${ssrInterpolate(formatDate(review.created_at))}</span></div>`);
          if (__props.currentUser && review.user_id === __props.currentUser.id) {
            _push(`<div class="flex space-x-2" data-v-a4346698><button class="text-blue-500 hover:text-blue-700 text-sm font-medium" data-v-a4346698> Редагувати </button><button class="text-red-500 hover:text-red-700 text-sm font-medium" data-v-a4346698> Видалити </button></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
          if (review.title) {
            _push(`<h4 class="text-lg font-semibold text-gray-700 mb-1" data-v-a4346698>${ssrInterpolate(review.title)}</h4>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<p class="text-gray-600 leading-relaxed" data-v-a4346698>${ssrInterpolate(review.review)}</p></div>`);
        });
        _push(`<!--]-->`);
        if (hasMoreReviews.value) {
          _push(`<button${ssrIncludeBooleanAttr(unref(api).loading.value) ? " disabled" : ""} class="mt-4 w-full py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed" data-v-a4346698>`);
          if (unref(api).loading.value) {
            _push(`<span data-v-a4346698>Завантаження...</span>`);
          } else {
            _push(`<span data-v-a4346698>Завантажити більше</span>`);
          }
          _push(`</button>`);
        } else {
          _push(`<!---->`);
...
const ReviewList = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-a4346698"]]);
const _sfc_main$2 = {
  __name: "SchemaMarkup",
  __ssrInlineRender: true,
  setup(__props) {
...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><div class="grid grid-cols-2 md:grid-cols-12 w-full h-full gap-2" data-v-1731564f><div class="col-span-5 h-[75vh]" data-v-1731564f>`);
      _push(ssrRenderComponent(MediaSlider, {
        images: product.value.images
      }, null, _parent));
      _push(`</div><div class="col-span-7 h-full flex flex-col bg-gray-200 gap-[1px]" data-v-1731564f><div class="p-12 flex flex-col bg-white" data-v-1731564f><h2 class="uppercase text-gray-400 text-xs tracking-widest mb-2 font-[&#39;Exo_2&#39;]" data-v-1731564f>${ssrInterpolate(product.value?.category?.title)}</h2><h1 class="font-semibold text-3xl leading-tight" data-v-1731564f>${ssrInterpolate(product.value?.seo?.title || product.value.title || "Товар")}</h1><div class="mt-4" data-v-1731564f>`);
      _push(ssrRenderComponent(_sfc_main$6, {
        "read-only": true,
        "show-average": true,
      ...
      _push(`</div></div><div class="p-12 bg-white flex flex-wrap items-center gap-8" data-v-1731564f><div data-v-1731564f><div class="mb-1 h-5" data-v-1731564f>`);
      if (product.value?.quantity > 0) {
        _push(`<span class="text-[10px] text-green-600 font-bold uppercase tracking-tighter" data-v-1731564f>${ssrInterpolate(_ctx.$t("inStoke"))}</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div><div class="flex flex-col" data-v-1731564f>`);
      if (product.value.old_price) {
        _push(`<span class="text-sm text-gray-400 line-through -mb-1" data-v-1731564f>${ssrInterpolate(product.value.old_price)}₴ </span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<span class="text-3xl font-bold font-[&#39;Exo_2&#39;]" data-v-1731564f>${ssrInterpolate(product.value.price)}₴ </span></div></div><div class="flex items-center border border-gray-200 rounded-lg h-14 bg-gray-50" data-v-1731564f><button class="px-4 h-full hover:bg-white transition-colors disabled:opacity-30 border-r"${ssrIncludeBooleanAttr(quantity.value <= 1) ? " disabled" : ""} data-v-1731564f>`);
      _push(ssrRenderComponent(unref(IconMinus), {
        size: 18,
        "stroke-width": "1.5"
      }, null, _parent));
      _push(`</button><input${ssrRenderAttr("value", quantity.value)} type="number" readonly class="w-14 text-center border-none bg-transparent focus:ring-0 font-[&#39;Exo_2&#39;] font-bold text-lg" data-v-1731564f><button class="px-4 h-full hover:bg-white transition-colors border-l" data-v-1731564f>`);
      _push(ssrRenderComponent(unref(IconPlus), {
        size: 18,
        "stroke-width": "1.5"
      }, null, _parent));
      _push(`</button></div><div class="flex-grow md:flex-grow-0" data-v-1731564f>`);
      ...
      _push(`</div></div><div class="p-12 bg-white" data-v-1731564f><h3 class="font-bold uppercase text-xs tracking-widest mb-6" data-v-1731564f> Залишити відгук </h3>`);
      _push(ssrRenderComponent(_sfc_main$4, {
        "model-id": product.value.id,
        "model-type": "Item"
      }, null, _parent));
      _push(`</div><div class="p-12 bg-white" data-v-1731564f><h3 class="font-bold uppercase text-xs tracking-widest mb-6 text-gray-400" data-v-1731564f> Відгуки клієнтів </h3>`);
      _push(ssrRenderComponent(ReviewList, {
        "model-id": product.value.id,
        "model-type": "Item"
      }, null, _parent));
      _push(`</div><div class="p-12 bg-white flex-grow" data-v-1731564f><h3 class="font-bold uppercase text-xs tracking-widest mb-4" data-v-1731564f> Опис товару </h3><div class="prose prose-sm text-gray-600 leading-relaxed" data-v-1731564f>${ssrInterpolate(product.value.description)}</div></div></div><div class="p-6 h-full col-span-2" data-v-1731564f></div><div class="p-6 h-full col-span-full bg-white border-t border-gray-100" data-v-1731564f>`);
      if (product.value?.images?.length > 1) {
        _push(`<div class="grid grid-cols-2 md:grid-cols-4 gap-4" data-v-1731564f><!--[-->`);
        ssrRenderList(product.value.images, (img, index) => {
          _push(`<div class="aspect-square bg-gray-50 rounded-lg overflow-hidden flex items-center justify-center p-4" data-v-1731564f><img${ssrRenderAttr("src", img?.url)} class="max-w-full max-h-full object-contain mix-blend-multiply"${ssrRenderAttr("alt", product.value.title)} data-v-1731564f></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div><div class="bg-gray-100 rounded-lg p-12 overflow-hidden col-span-full" data-v-1731564f><h4 class="text-xs font-mono text-gray-400 mb-4 uppercase tracking-widest" data-v-1731564f> Debug Info </h4><pre class="text-[10px] leading-tight text-gray-500 overflow-x-auto" data-v-1731564f>${ssrInterpolate(product.value)}</pre></div></div>`);
...
const View = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-1731564f"]]);
export {
  View as default
};
