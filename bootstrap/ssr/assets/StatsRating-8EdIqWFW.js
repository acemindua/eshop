  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.averageReviews.rating) {
        _push(`<div class="flex items-center gap-1 group cursor-pointer">`);
        ...
        _push(`<span class="md:hidden font-medium text-gray-800">${ssrInterpolate(__props.averageReviews.rating)}</span><span class="text-gray-300">•</span><div class="flex items-center gap-1 text-gray-600 transition">`);
        _push(ssrRenderComponent(unref(IconMessage), {
          stroke: 1,
          class: "w-4 h-4"
        }, null, _parent));
        _push(`<span class="hidden md:block">${ssrInterpolate(unref(wTransChoice)("reviews", __props.averageReviews.total, {
          count: __props.averageReviews.total
        }))}</span><span class="md:hidden">${ssrInterpolate(__props.averageReviews.total)}</span></div></div>`);
      } else {
