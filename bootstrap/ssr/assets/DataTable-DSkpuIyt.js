    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.items.length > 0) {
        ...
        ssrRenderList(__props.items, (item, key) => {
          ...
          _push(`</td><td class="p-2"><p class="font-semibold">${ssrInterpolate(item.title)}</p><p class="text-xs text-gray-500 hover:text-indigo-400 duration-150 transition">`);
          if (_ctx.$page.props?.app?.url && item.slug) {
            _push(`<a${ssrRenderAttr("href", `${_ctx.$page.props?.app?.url}/${item.slug}`)} target="_blank" class="inline-flex items-center gx-1">`);
            _push(ssrRenderComponent(unref(IconLink), {
              stroke: 2,
              size: 14,
              class: "text-indigo-500"
            }, null, _parent));
            _push(`<span>${ssrInterpolate(`/${item.slug}`)}</span></a>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</p></td><td class="p-2 text-xs text-gray-500">${ssrInterpolate(item.category?.title)}</td><td class="p-2 text-end text-red-500">${ssrInterpolate(item.price)}</td><td class="p-2"><div class="flex justify-center">`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Commerce/Items/Partials/DataTable.vue");
