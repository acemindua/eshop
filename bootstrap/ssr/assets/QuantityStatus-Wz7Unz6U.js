  ...
  setup(__props) {
    ...
    const status = computed(() => {
      ...
      return {
        ...
        label: isInStock ? "inStock" : "outOfStock",
        textColor: isInStock ? "text-gray-800" : "text-gray-400",
        iconColor: isInStock ? "text-green-600" : "text-orange-400"
      };
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: ["flex items-center gap-1.5 text-sm font-medium", status.value.textColor]
      }, _attrs))}>`);
