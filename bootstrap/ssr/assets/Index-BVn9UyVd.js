import { computed, ref, watch, mergeProps, unref, withCtx, createVNode, toDisplayString, openBlock, createBlock, createCommentVNode, withModifiers, Fragment, useSSRContext } from "vue";
...
import { _ as _sfc_main$3 } from "./DataTable-WSbj1FXl.js";
...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: _sfc_main$1 }, {
  ...
  props: {
    data: { type: Object },
    filters: { type: Object },
    prefix: { type: String, default: "settings.commerce.warehouses" }
  },
  setup(__props) {
    const props = __props;
    const model = props.prefix;
    const items = computed(() => props.data?.warehouses?.data || []);
    const metaItems = computed(() => props.data?.warehouses?.meta || []);
    ...
    const handleToggleActive = (item) => {
      router.patch(
        route(`admin.${model}.toggle`, item.id),
        {},
        {
          preserveScroll: true,
          preserveState: true
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "max-w-7xl mx-auto space-y-6" }, _attrs))}><div class="flex flex-col space-y-4"><div class="flex items-center justify-between bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm"><div class="flex items-center space-x-3"><div class="p-2 bg-slate-900 text-white rounded-xl shadow-sm">`);
      _push(ssrRenderComponent(unref(IconBuildingWarehouse), { size: "20" }, null, _parent));
      _push(`</div><div><h2 class="text-base font-black text-slate-900 tracking-tight">${ssrInterpolate(_ctx.$t("Warehouses"))}</h2><p class="text-xs text-slate-400 font-medium">${ssrInterpolate(_ctx.$t(
        "Manage logistics locations, addresses, and working schedules"
      ))}</p></div></div>`);
      _push(ssrRenderComponent(ButtonsGroup, { buttons: actionButtons }, null, _parent));
      _push(`</div><section class="flex items-center gap-3">`);
