...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</th><th class="p-3 text-center w-24">#</th><th class="py-3 text-center w-24">${ssrInterpolate(_ctx.$t("Media"))}</th><th class="p-3 text-start">${ssrInterpolate(_ctx.$t("Title"))}</th><th class="p-3 text-start">${ssrInterpolate(_ctx.$t("Parent"))}</th><th class="p-3 text-center w-48">${ssrInterpolate(_ctx.$t("Status"))}</th><th class="p-3 w-48">`);
      ...
      if (__props.items.length > 0) {
        ...
        ssrRenderList(__props.items, (item, key) => {
          ...
          _push(`</td><td class="p-2 text-center text-gray-500">${ssrInterpolate(_ctx.$formatSerial(count.value + key))}</td><th class="p-1"><div>`);
          _push(ssrRenderComponent(_sfc_main$2, {
            src: item?.image || null
          }, null, _parent));
          _push(`</div></th><td class="p-2"><p class="font-semibold">${ssrInterpolate(item.title)}</p><p class="text-xs text-gray-500 hover:text-indigo-400 duration-150 transition">${ssrInterpolate(`/${item.slug}`)}</p></td><td class="p-2 text-xs text-gray-500">${ssrInterpolate(item.parent?.title || "—")}</td><td class="p-2"><div class="flex justify-center">`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Commerce/Categories/Partials/DataTable.vue");
