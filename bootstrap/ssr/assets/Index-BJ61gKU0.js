...
const _sfc_main = /* @__PURE__ */ Object.assign({ layout: _sfc_main$1 }, {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div><h1 class="text-base font-semibold tracking-tight dark:text-slate-200">${ssrInterpolate(_ctx.$t("Items"))}</h1><p class="text-xs text-slate-400 font-medium">${ssrInterpolate(_ctx.$t("Manage item content and structure"))}</p></div></div>`);
      ...
      _push(ssrRenderComponent(_sfc_main$3, {
        ...
        headers: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`</div></th><th class="p-2 text-center"${_scopeId}>${ssrInterpolate(_ctx.$t("Category"))}</th><th class="p-2 text-center"${_scopeId}>${ssrInterpolate(_ctx.$t("Price"))}</th><th class="p-2 text-center"${_scopeId}>${ssrInterpolate(_ctx.$t("Quantity"))}</th><th class="p-2 text-center w-48"${_scopeId}>${ssrInterpolate(_ctx.$t("Status"))}</th>`);
          } else {
            return [
              ...
              createVNode("th", { class: "p-2 text-center" }, toDisplayString(_ctx.$t("Category")), 1),
              createVNode("th", { class: "p-2 text-center" }, toDisplayString(_ctx.$t("Price")), 1),
              createVNode("th", { class: "p-2 text-center" }, toDisplayString(_ctx.$t("Quantity")), 1),
        ...
        "item-data": withCtx(({ item }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`</td><td class="p-2"${_scopeId}><p class="font-semibold"${_scopeId}>${ssrInterpolate(item.title)}</p><p class="text-xs text-gray-500 hover:text-indigo-400 duration-150 transition"${_scopeId}> url: ${ssrInterpolate(`/${item.slug}`)}</p></td><td class="p-2"${_scopeId}><span class="text-gray-600 italic"${_scopeId}>${ssrInterpolate(item.category ? item.category.title : "")}</span></td><td class="p-2"${_scopeId}>${ssrInterpolate(item.price)}</td><td class="p-2 flex items-center justify-center"${_scopeId}><span class="${ssrRenderClass([
              item.quantity <= 10 ? "bg-yellow-100 border-yellow-200 text-red-700" : "bg-green-50 border-green-100 text-green-600",
              "block border px-2 text-sm rounded-lg"
            ])}"${_scopeId}>${ssrInterpolate(item.quantity)}</span></td><td class="p-2"${_scopeId}><div class="flex justify-center"${_scopeId}>`);
            ...
            return [
              ...
              createVNode("td", { class: "p-2" }, [
                createVNode("p", { class: "font-semibold" }, toDisplayString(item.title), 1),
                createVNode("p", { class: "text-xs text-gray-500 hover:text-indigo-400 duration-150 transition" }, " url: " + toDisplayString(`/${item.slug}`), 1)
              ]),
              createVNode("td", { class: "p-2" }, [
                createVNode("span", { class: "text-gray-600 italic" }, toDisplayString(item.category ? item.category.title : ""), 1)
              ]),
              createVNode("td", { class: "p-2" }, toDisplayString(item.price), 1),
              createVNode("td", { class: "p-2 flex items-center justify-center" }, [
                createVNode("span", {
                  class: [
                    "block border px-2 text-sm rounded-lg",
                    item.quantity <= 10 ? "bg-yellow-100 border-yellow-200 text-red-700" : "bg-green-50 border-green-100 text-green-600"
                  ]
                }, toDisplayString(item.quantity), 3)
              ]),
              createVNode("td", { class: "p-2" }, [
