...
import BuyButton from "./BuyButton-JQnkdb_2.js";
import _sfc_main$3 from "./QuantityStatus-Wz7Unz6U.js";
import _sfc_main$1 from "./StatsRating-Cjhsk8Tq.js";
import _sfc_main$2 from "./ProductPrice-BQZjVJ64.js";
import "@vue/shared";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const hasDiscount = computed(() => {
      const old = Number(props.data?.old_price);
      const curr = Number(props.data?.price);
      return old > curr;
    });
    const discountPercentage = computed(() => {
      if (!hasDiscount.value) return 0;
      return Math.round(
        (props.data.old_price - props.data.price) / props.data.old_price * 100
      );
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative group h-full w-full overflow-hidden border-b border-r p-4" }, _attrs))}>`);
      ...
      _push(`<div class="flex flex-col"><div class="bg-gray-100 text-gray-300 flex items-center justify-center h-64 relative z-10 w-full">`);
      ...
      _push(`</div><h3 class="mt-4 text-sm font-medium text-gray-800 transition-colors group-hover:underline line-clamp-2 h-12">${ssrInterpolate(__props.data.title)}</h3>`);
      _push(ssrRenderComponent(_sfc_main$1, {
        "average-reviews": __props.data.average_reviews || {}
      }, null, _parent));
      _push(`<div class="grid grid-cols-2 h-12"><div class="flex items-center">`);
      ...
      _push(`</div></div><div class="flex items-center justify-between"></div>`);
