...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex items-center gap-1.5 relative z-20" }, _attrs))} data-v-1ab100c4><!--[-->`);
      ssrRenderList(__props.buttons, (btn, index) => {
        ssrRenderVNode(_push, createVNode(resolveDynamicComponent(btn.type === "primary" ? PrimaryButton : _sfc_main$2), mergeProps({ ref_for: true }, btn.extraProps, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="text-[12px] sm:text-sm font-bold tracking-tight whitespace-nowrap" data-v-1ab100c4${_scopeId}>${ssrInterpolate(__props.loading && btn.loadingLabel ? _ctx.$t(btn.loadingLabel) : _ctx.$t(btn.label))}</span>`);
              if (btn.type === "primary") {
                _push2(`<div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:animate-[shimmer_2s_infinite] pointer-events-none" data-v-1ab100c4${_scopeId}></div>`);
              } else {
                _push2(`<!---->`);
              }
            } else {
...
const ButtonsGroup = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-1ab100c4"]]);
export {
  ButtonsGroup as B
};
