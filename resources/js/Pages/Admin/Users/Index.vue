                ...
                <template #item-data="{ item }">
                    <td class="p-2 text-center">
                        <div class="relative inline-flex">
                            <ImageDataView :src="item?.avatar" :configs="{ width: '32px', height: '32px' }" class="rounded-full shadow-sm" />
                        </div>
                    </td>
                    ...
                    <td class="p-2">
                        <div class="flex flex-col leading-tight">
                            ...
                            <UserStatus 
                                :user="{ is_online: item.online, last_activity: item.last_activity_raw }" 
                                size="xs" 
                            />
                        </div>
                    </td>
                    ...
                    <td class="p-2">
                        <div class="flex flex-col text-xs space-y-0.5">
                            <span class="text-slate-700 font-medium">{{ formatPhone(item.phone) || '—' }}</span>
                            <span class="text-slate-400 italic text-[10px]">{{ item.email }}</span>
                        </div>
                    </td>

                    <td class="p-2 text-center font-mono text-xs">
                        <span :class="item.bonus_balance > 0 ? 'text-brand font-bold' : 'text-slate-400'">
                            {{ item.bonus_balance }} ₴
                        </span>
                    </td>
                </template>
