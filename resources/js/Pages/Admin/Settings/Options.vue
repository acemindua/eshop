...
const emptyCounts = computed(() => {
    ...
    localesKeys.value.forEach((locale) => {
        counts[locale] = form.items.filter(
            (item) =>
                !item.values[locale] ||
                String(item.values[locale]).trim() === "",
        ).length;
    });
    return counts;
});
    ...
    if (filterStatus.value === "empty") {
        items = items.filter(
            (item) =>
                ...
                String(item.values[globalLocale.value]).trim() === "",
        );
    }
...
const getCountryCode = (r) =>
    r && r.includes("_") ? r.split("_")[1].toLowerCase() : "ua";
const isFieldEmpty = (item, lang) =>
    !item.values[lang] || String(item.values[lang]).trim() === "";
    ...
    <div class="flex flex-col space-y-4">
        ...
        <section class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div
                    class="inline-flex rounded-lg border border-slate-200 p-0.5 bg-white select-none"
                >
                    <button
                        type="button"
                ...
                <div
                    class="flex items-center gap-1 border-l border-slate-200 pl-3 select-none"
                >
                    <button
                    ...
                    >
                        ...
                        />
                        <span class="uppercase tracking-wider text-[11px]">{{
                            code
                        }}</span>

                        <!-- Червона мітка незаповнених полів локалі -->
                        <span
                            v-if="emptyCounts[code] > 0"
        ...
        <section
            class="space-y-6 overflow-y-auto max-h-[70vh] pr-1 scrollbar-thin"
        >
            <div
            ...
            >
                ...
                <div class="flex items-center gap-2 px-1 pt-2">
                    <div
                        class="p-1 rounded bg-slate-100 text-slate-600 border border-slate-200/60"
                    >
                        <IconFolder size="14" />
                    </div>
                    <span
                    ...
                    >
                    <div
                        class="h-px bg-gradient-to-r from-slate-200 to-transparent flex-1 ml-2"
                    ></div>
                </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-3">
                    <div
                        ...
                        :class="[
                            isFieldEmpty(item, globalLocale)
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="flex items-center gap-1 mt-1">
                                <span
                                    v-for="lKey in localesKeys"
                                    :key="lKey"
                                    class="w-3 h-1.5 rounded-sm transition-all"
                                    :class="
                                        isFieldEmpty(item, lKey)
                                            ? 'bg-slate-200'
                                            : 'bg-green-500'
                                    "
                        ...
                        <div class="relative w-full mt-1 flex items-center">
                            <textarea
                                v-model="item.values[globalLocale]"
                                rows="1"
                                ...
                                :placeholder="`Текст для [${globalLocale}]...`"
                                @input="adjustHeight"
                            ></textarea>

                            <!-- Внутрішні контроли поля -->
                            <div
                                class="absolute right-2.5 flex items-center space-x-1.5 select-none"
                            >
                                <button
                                    v-if="item.values[globalLocale]"
                                    type="button"
                                    @click="clearField(item)"
                                ...
                                <span
                                    class="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-100 text-slate-400 border border-slate-200"
                                >
                                    {{ globalLocale }}
                                </span>
                            </div>
            ...
            >
                ...
                <span class="font-semibold text-slate-700"
                    >Перекладів не знайдено</span
                >
                <p class="text-xs text-slate-400">
                    Змініть параметри пошуку або перевірте іншу мову.
                </p>
            </div>
        </section>
    </div>
