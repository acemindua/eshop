...
<template>
    ...
    <Head :title="`${title} - ${page.props.app?.name}` ?? 'App'" />

    <div v-if="!isSidebarVisible" @click="isSidebarVisible = true"
        class="fixed inset-0 bg-black/40 z-40 md:hidden transition-opacity duration-300" />

    <div class="bg-gray-100 dark:bg-slate-800 text-gray-900 dark:text-slate-200 w-full min-h-screen">
        <div class="flex w-full h-full relative">
            <aside class="border-r dark:border-slate-800 dark:shadow-2xl" :class="[
                isSidebarVisible
                    ? 'hidden md:flex relative'
                    : 'flex absolute h-full',
            ]">
                <Transition name="sidebar-slide-fade">
                    <div class="bg-gray-50 dark:bg-slate-900 flex flex-col z-[60]">
                        <AdminSidebar />
                    </div>
                </Transition>
                ...
                <Transition name="sidebar-slide-fade">
                    <div v-if="$slots.sidenav" v-show="isSecondarySidebarVisible"
            ...
            <div class="flex flex-col w-full h-screen overflow-y-auto">
                <header class="w-full h-14 border-b text-xs text-gray-600 dark:text-slate-300 dark:border-slate-700">
                    <div class="px-2 md:px-4 lg:px-8 h-full">
                        <div class="flex justify-between items-center h-full space-x-2 border-l border-r">
                            <div class="flex items-center">
                                <button @click.prevent="
                                    ...
                                    <svg class="w-6 h-6 fill-current" viewBox="0 0 16 16">
                                        <rect class="origin-center" y="3" width="16" height="2" rx="1" />
                                        <rect class="origin-center" y="7" width="16" height="2" rx="1" />
                                        <rect class="origin-center" y="11" width="16" height="2" rx="1" />
                                    </svg>
                                </button>

                                <div class="p-2 ml-2">
                                    <h1 class="font-semibold text-gray-800 dark:text-slate-300 text-base">
                                        {{ title }}
                                    </h1>
                                    <Breadcrumbs />
                                </div>
                            ...
                            <Link v-tooltip="{
                                content: $t('Log out'),
                                placement: 'bottom',
                            }" method="post" href="/logout" class="px-4 hover:text-gray-800 duration-150">
                            <IconLogout stroke="{2}" />
                            </Link>
                        </div>
                    </div>
                ...
                <footer class="text-xs text-gray-600 dark:text-slate-400 border-t dark:border-slate-700">
                    <div class="px-2 md:px-4 lg:px-8">
                        <p class="text-center md:text-end border-l border-r h-full p-4">
